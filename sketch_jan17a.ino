...
*/
  ...
  int x = 25;            // initialize variable at one
...
void loop() {     // repeat in a loop
  ...
  prizm.setMotorPower(2,x);     // spin Motor 1 CW at 25% power
  delay(1000);                  // wait while the motor runs for 5 seconds
  ...
  prizm.setMotorPower(2,-x);    // spin Motor 1 CCW at 25% power
  delay(1000);                  // wait while the motor runs for 5 seconds
  prizm.setMotorPower(1,0);     // stop motor (coast to stop)
  prizm.setMotorPower(2,0);     // stop motor (coast to stop)
